...
class FakeProvider:
	...
	var provider_id_value := "mediapipe_python"
	var supported_capabilities: Array = []

	func _init(provider_id: String = "mediapipe_python", capabilities: Array = []) -> void:
...
func test_publish_and_request_session_by_provider_and_capability() -> void:
	var provider: FakeProvider = add_child_autoqfree(
		FakeProvider.new("mediapipe_python", [
	...
	assert_eq(String(session.get("session_key", "")), "mediapipe_python/desktop_main")
...
func test_acquire_and_release_track_borrowers_without_transferring_ownership() -> void:
	var provider: FakeProvider = add_child_autoqfree(
		FakeProvider.new("mediapipe_python", [AeroInputProvider.Capability.GESTURE_RECOGNITION])
	)
	AeroProviderSessionRegistry.publish_session(
		"assembly:testbed",
		provider,
		{"session_key": "mediapipe_python/shared"}
	)
	...
	var release := AeroProviderSessionRegistry.release_session("camera_gesture:testbed", "mediapipe_python/shared")
...
func test_unpublish_requires_same_owner_id() -> void:
	...
	var wrong_owner := AeroProviderSessionRegistry.unpublish_session("camera_gesture_consumer", "mediapipe_python/shared")
	...
	var request_after_failure := AeroProviderSessionRegistry.request_session({"session_key": "mediapipe_python/shared"})
	assert_true(bool(request_after_failure.get("ok", false)))

	var correct_owner := AeroProviderSessionRegistry.unpublish_session("mediapipe_owner", "mediapipe_python/shared")
	...
	assert_false(bool(AeroProviderSessionRegistry.request_session({"session_key": "mediapipe_python/shared"}).get("ok", false)))

func test_request_filters_by_exact_owner_and_metadata() -> void:
	var owner_a: FakeProvider = add_child_autoqfree(FakeProvider.new("mediapipe_python"))
	var owner_b: FakeProvider = add_child_autoqfree(FakeProvider.new("mediapipe_python"))
	AeroProviderSessionRegistry.publish_session(
		"owner_a",
		owner_a,
		{
			"session_key": "mediapipe_python/owner_a",
	...
	AeroProviderSessionRegistry.publish_session(
		...
		owner_b,
		{
			"session_key": "mediapipe_python/owner_b",
	...
	assert_true(bool(request.get("ok", false)))
	assert_eq(String(request.get("session", {}).get("session_key", "")), "mediapipe_python/owner_b")
